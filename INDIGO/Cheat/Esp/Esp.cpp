				...
				if ( pModelName )
				{
					...
					if ( WorldToScreen( pEntity->GetRenderOrigin() , vEntScreen ) )
					{
						...
						if ( Settings::Esp::esp_WorldWeapons && !strstr( pModelName , "models/weapons/w_eq_" )
							 && !strstr( pModelName , "models/weapons/w_ied" ) ) // бомб?не оружие
						{
							 ...
							 strstr( pModelName , "models/Weapons/w_eq_" ) ) )
						{
							if ( strstr( pModelName , "_dropped.mdl" ) )
							{
								...
								Color GrenadeColor = Color::White(); // по умолчани?decoy - эт?ложн?

								if ( strstr( pModelName , "fraggrenade" ) || strstr( pModelName , "molotov" ) ) // осколочн? ил?молото?
								{
									GrenadeColor = Color::Red();
								}
								else if ( strstr( pModelName , "flashbang" ) ) // светов?
								{
